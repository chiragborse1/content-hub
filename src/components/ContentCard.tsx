...
import { ExternalLink, FileText, Pencil, Loader2, ChevronDown, Trash2 } from "lucide-react";
    ...
    >
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-semibold text-sm text-foreground line-clamp-1">{item.title || "Untitled"}</h3>
        ...
        <div className="flex gap-2 pt-1" onClick={(e) => e.stopPropagation()}>
          ...
          {/* Extract */}
          <Button variant="outline" size="sm" className="flex-1 text-xs h-8" disabled={extracting} onClick={handleExtract}>
          ...
          <div className="relative flex-1">
            <Button variant="outline" size="sm" className="w-full text-xs h-8 justify-between" disabled={savingStatus} onClick={() => setStatusOpen((o) => !o)}>
          ...
          {/* Edit */}
          <Button variant="outline" size="sm" className="text-xs h-8" onClick={() => navigate(`/content/${item.id}`)}>
          ...
          <Button
            variant="outline"
            size="sm"
            className="text-xs h-8 text-destructive hover:bg-destructive hover:text-destructive-foreground transition-colors"
            disabled={deleting}
